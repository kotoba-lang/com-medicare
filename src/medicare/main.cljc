(ns medicare.main
  "Kotodama WASM entrypoint for the Medicare clean-room actor (L4) — Clojure port.
  ...
  facts (`medicare.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "medicare")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "medicare_fil"
    :fields [:filerId :form :period :status]           :required [:form :period]
    :coerce {}                          :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "medicare_ent"
    :fields [:name :ein :jurisdiction]                 :required [:name :ein]
    :coerce {}                          :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "medicare_dat"
    :fields [:name :agency :updatedAt]                 :required [:name :agency]
    :coerce {}                          :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "medicare_app"
    :fields [:applicantId :program :status]            :required [:program :status]
    :coerce {}                          :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "medicare_pub"
    :fields [:recordType :jurisdiction :recordedAt]    :required [:recordType :jurisdiction]
    :coerce {}                          :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "medicare_not"
    :fields [:subjectId :type :issuedAt]               :required [:type :issuedAt]
    :coerce {}                          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"medicare.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "medicare-compat" :tier tier :entities entities} 200])
...
(defn start! [] :medicare-compat/ready)
